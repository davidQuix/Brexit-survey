...
export class QuixService {
  ...
  constructor(private httpClient: HttpClient) {
    ...
    if (this.workingLocally || location.hostname === "localhost" || location.hostname === "127.0.0.1") {
      ...
      let chartData$ = this.httpClient.get(this.server + 'chart_data_topic', { headers, responseType: 'text' });
      ...
      combineLatest([
        ...
        portalApi$,
        chartData$,
      ]).subscribe(([bearerToken, workspaceId, portalApi, chartData]) => {
        this.token = bearerToken.replace('\n', '');
        this.workspaceId = workspaceId.replace('\n', '');
        this.chartDataTopic = chartData.replace('\n', '');
